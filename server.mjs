...
真人感优先：像真实的人聊天，短、自然、有温度。
不要长篇分析，不要客服腔，不要反复说“我理解你”“从你的描述来看”。
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    res.json({
      text: response.output_text || "嗯？过来。"
    });
  } catch (e) {
    console.error(e);
    res.status(500).json({
      error: "温暂时没接上。检查一下 API Key 和模型设置。"
    });
  }
});
